# Master Prompt v2.1: GenAi Aggressive Stock Portfolio Management System
...
GenAi Aggressive Portfolio: Weekly Performance Review – Quantum Investor Digest
...
**CRITICAL: Use exact HTML/CSS template with brand purple header (#8B7AB8)**
...
.myblock-portfolio-table th:first-child,
.myblock-portfolio-table td:first-child {
  text-align: left;
}
...
<table class="myblock-portfolio-table" aria-label="Portfolio performance comparison">
  ...
  <tbody>
    <tr style="background: white;">
      <td style="padding: 14px 16px; border-bottom: 1px solid #E8E8E8; font-weight: 600;">GenAi Chosen ($)</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8;">10,000</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8;">[Previous Week Value]</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8;">[Current Week Value]</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8; color: [#2E7D32 or #C62828]; font-weight: 600;">[Weekly Change %]</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8; color: [#2E7D32 or #C62828]; font-weight: 600;">[Total Return %]</td>
    </tr>
    <tr>
      <td style="padding: 14px 16px; border-bottom: 1px solid #E8E8E8; font-weight: 600;">S&P 500 (Index)</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8;">6,688</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8;">[Previous Week Level]</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8;">[Current Week Level]</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8; color: [#2E7D32 or #C62828]; font-weight: 600;">[Weekly Change %]</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8; color: [#2E7D32 or #C62828]; font-weight: 600;">[Total Return %]</td>
    </tr>
    <tr style="background: white;">
      <td style="padding: 14px 16px; border-bottom: 1px solid #E8E8E8; font-weight: 600;">Bitcoin ($)</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8;">123,353</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8;">[Previous Week Price]</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8;">[Current Week Price]</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8; color: [#2E7D32 or #C62828]; font-weight: 600;">[Weekly Change %]</td>
      <td style="padding: 14px 16px; text-align: right; border-bottom: 1px solid #E8E8E8; color: [#2E7D32 or #C62828]; font-weight: 600;">[Total Return %]</td>
    </tr>
  </tbody>
...
- **Font:** font-family: inherit (matches blog typography)
...
- **Same styling approach** as tables (myblock- prefix, clean modern design)
...
<div class="myblock-chart-container">
  ...
  <div class="myblock-chart-wrapper">
    <svg class="myblock-chart-svg" viewBox="0 0 900 400" preserveAspectRatio="xMidYMid meet">
      ...
      <text class="myblock-chart-label" x="65" y="130" text-anchor="end">[Mid-High Value]</text>
      <text class="myblock-chart-label" x="65" y="205" text-anchor="end">100</text>
      <text class="myblock-chart-label" x="65" y="280" text-anchor="end">[Mid-Low Value]</text>
      <text class="myblock-chart-label" x="65" y="355" text-anchor="end">[Min Value]</text>
      
      <!-- X-axis labels (dates for each data point) -->
      ...
      <!-- GenAi Portfolio Line (Purple) - update points weekly -->
      <polyline class="myblock-chart-line-genai" 
                points="[x1,y1 x2,y2 x3,y3 ...]"/>
      
      <!-- S&P 500 Line (Green dashed) - update points weekly -->
      <polyline class="myblock-chart-line-sp500" 
                points="[x1,y1 x2,y2 x3,y3 ...]"/>
      
      <!-- Bitcoin Line (Red dotted) - update points weekly -->
      <polyline class="myblock-chart-line-bitcoin" 
                points="[x1,y1 x2,y2 x3,y3 ...]"/>
      
      <!-- GenAi Portfolio Dots (add one per week) -->
      <circle class="myblock-chart-dot myblock-chart-dot-genai" cx="[x]" cy="[y]"/>
      
      <!-- S&P 500 Dots (add one per week) -->
      <circle class="myblock-chart-dot myblock-chart-dot-sp500" cx="[x]" cy="[y]"/>
      
      <!-- Bitcoin Dots (add one per week) -->
      <circle class="myblock-chart-dot myblock-chart-dot-bitcoin" cx="[x]" cy="[y]"/>
      
      <!-- Value labels at key GenAi portfolio points -->
      <text class="myblock-chart-label" x="[x]" y="[y]" text-anchor="middle" style="font-weight: 600; fill: #8B7AB8;">[Normalized Value]</text>
    </svg>
  </div>
...
3. **Inconsistent Data:** Ensure this week's "previous week" matches last week's "current week"
4. **Missing Sections:** Never omit Portfolio Update, Table, Chart, Recommendation, or Risk Disclosure
5. **Calculation Errors:** Double-check all percentage calculations
6. **Removed Sections:** Never include "Comparative Chart" or "Join the Conversation" sections (these were removed)
7. **Missing Transformer Reference:** Always mention "transformer-based AI model" in introduction
8. **Wrong CSS Classes:** Always use "myblock-" prefix, never generic class names
9. **Non-Responsive Design:** Always include responsive breakpoints for mobile
10. **Chart Not Normalized:** Always show chart with base 100 at inception, not raw values
...
- **Light Gray:** #E8E8E8 (borders, grid lines)
- **Background White:** #FFFFFF
- **Alternate Row:** #F9F9F9 (table zebra striping)
- **Hover Gray:** #F0F0F0 (table row hover)
...
**Current Version:** Master Prompt v2.1
**Last Updated:** November 6, 2025, 6:00 PM EST
...
3. **Generate Blog Post** → Use exact format with introduction, holdings list, table, chart, analysis, recommendation, risk disclosure
